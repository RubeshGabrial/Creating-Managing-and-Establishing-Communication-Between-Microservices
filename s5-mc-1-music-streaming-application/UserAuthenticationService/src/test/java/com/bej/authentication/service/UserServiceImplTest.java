...
class UserServiceImplTest {
    ...
    {
        when(userRepository.findByUserIdAndPassword(anyString(),any())).thenReturn(user);

        assertEquals(user,userRepository.findByUserIdAndPassword(user.getUserId(),user.getPassword()));
        verify(userRepository,times(1)).findByUserIdAndPassword(anyString(),any());
    }
