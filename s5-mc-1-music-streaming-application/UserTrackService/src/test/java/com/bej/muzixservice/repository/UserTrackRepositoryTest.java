...
class UserTrackRepositoryTest {
    ...
    void givenUserTrackToSaveShouldReturnSavedTrack() {
        ...
        Optional<User> retrievedUser = userTrackRepository.findById(user.getUserId());
    ...
    public void givenTrackToDeleteShouldDeleteTrack() {
        ...
        Optional<User> retrievedUser = userTrackRepository.findById(user.getUserId());
        assertTrue(retrievedUser.isPresent(), "User should be present before deletion");

        userTrackRepository.delete(retrievedUser.get());
        Optional<User> userAfterDeletion = userTrackRepository.findById(user.getUserId());
        assertFalse(userAfterDeletion.isPresent(), "User should not be present after deletion");
    }
